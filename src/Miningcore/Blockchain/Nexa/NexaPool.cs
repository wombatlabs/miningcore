...
public class NexaPool : PoolBase
{
    ...
    private object CreateWorkerJob(StratumConnection connection, bool cleanJob)
    {
        ...
        // if job has GetJobParams(bool), use it; else, fallback.
        var mi = job.GetType().GetMethod("GetJobParams", new[] { typeof(bool) });
        if (mi != null)
            return mi.Invoke(job, new object[] { cleanJob });
    ...
    protected virtual async Task OnNewJobAsync(object jobParams)
    {
        ...
        await Guard(() => ForEachMinerAsync(async (connection, ct) =>
        {
            ...
            // VarDiff pending: apply before sending new template
            if (context.ApplyPendingDifficulty())
                await connection.NotifyAsync(BitcoinStratumMethods.SetDifficulty, new object[] { context.Difficulty });

            // New block template: HERE it makes sense clean = true
    ...
    protected override async Task OnVarDiffUpdateAsync(StratumConnection connection, double newDiff, CancellationToken ct)
    {
        ...
        {
            // VarDiff: do NOT clean the job (prevents discarding work in progress)
